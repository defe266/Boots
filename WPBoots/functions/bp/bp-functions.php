...
function bp_tpack_enqueue_styles() {
	...
	wp_enqueue_style( 'bp', get_bloginfo('template_url') .'/functions/bp/css/'. $stylesheet, array(), $version );//plugins_url( '/bp-template-pack/' )
	wp_enqueue_style( 'bp-responsive', get_bloginfo('template_url') . '/functions/bp/css/responsive.css', array());

	// Enqueue RTL styles for BP 1.5+
	if ( version_compare( BP_VERSION, '1.3', '>' ) && is_rtl() )
		wp_enqueue_style( 'bp-rtl',  get_bloginfo('template_url') .'/functions/bp/css/' . 'bp-rtl.css', array( 'bp' ), $version );
}
...
if ( version_compare( BP_VERSION, '1.3', '<' ) ) :
	...
	//apply_filters( 'bp_search_form', "bingo" );
	
	/*
	add_filter("bp_search_form", "bpmag_remove_search_dropdown"  );
